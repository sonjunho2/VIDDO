    ...
    <main className="min-h-screen bg-[#070A12] text-white flex items-center justify-center px-6">
      ...
      <Card className="relative z-10 w-full max-w-md bg-[#0D1220]/90 border-white/10 rounded-3xl shadow-2xl">
        <CardContent className="p-8">
          ...
          <p className="text-zinc-400 mb-7">Start building AI videos with VIDDO.</p>
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div><label className="text-sm text-zinc-300">Password</label><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className="mt-2 w-full rounded-2xl bg-white/5 border border-white/10 px-4 py-3 text-white outline-none focus:border-blue-400" placeholder="At least 6 characters" /></div>
            {message && <p className="text-sm text-blue-200 bg-blue-500/10 border border-blue-400/20 rounded-2xl p-3">{message}</p>}
            <Button disabled={loading} className="w-full h-12 rounded-2xl bg-blue-500 hover:bg-blue-600 font-bold">{loading ? "Creating account..." : "Create Account"}</Button>
          </form>
          <p className="text-zinc-400 text-sm mt-6 text-center">Already have an account? <Link href="/login" className="text-blue-300 font-bold">Login</Link></p>
        </CardContent>
      </Card>
    </main>
  );
